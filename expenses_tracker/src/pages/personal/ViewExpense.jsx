...
function ExpenseRow({ expense, index, onEdit, onDelete }) {
  ...
  return (
    <tr className="border-b even:bg-blue-100 hover:bg-indigo-200 group">
...
// Mobile card view for a single expense. No hover affordance exists on
// touch devices, so edit/delete are always-visible icon buttons here
// instead of the group-hover pattern the desktop table uses.
function ExpenseCard({ expense, index, onEdit, onDelete }) {
  const [year, month, day] = expense.createdAt.split("-");
  const formatted = `${day}/${month}/${year}`;
  return (
    <div className="bg-white rounded-lg border border-indigo-200 shadow-sm p-4 flex flex-col gap-2">
      <div className="flex justify-between items-start gap-3">
        ...
        <p className="font-bold text-indigo-800 text-lg shrink-0">
          ₹{expense.amount}
        </p>
      </div>
      ...
      <div className="flex justify-end gap-1 pt-1 -mb-1 -mr-1">
        <button
          onClick={() => onEdit(index)}
          className="p-2 rounded-full hover:bg-indigo-100 active:bg-indigo-200"
        ...
        <button
          onClick={onDelete}
          className="p-2 rounded-full hover:bg-red-100 active:bg-red-200"
  ...
  return (
    <AppLayout bgClassName="bg-indigo-50">
      ...
      {expenses.length === 0 ? (
        ...
        <>
          {/* Mobile / tablet: stacked cards (no hover affordance, so
              edit/delete are always visible rather than group-hover) */}
          ...
          {/* Desktop: full table */}
          <div className="hidden md:block overflow-x-auto bg-white shadow-xl rounded-lg border border-indigo-200">
      ...
      {/* ---------- Edit Modal ---------- */}
      {editingIndex !== null && (
        <div className="fixed inset-0 bg-gray-400/25 flex items-center justify-center z-50 p-4">
          <div className="bg-white p-5 sm:p-6 rounded-2xl shadow-2xl w-full max-w-sm sm:max-w-md max-h-[90vh] overflow-y-auto">
            <h3 className="text-xl sm:text-2xl font-semibold mb-4 text-indigo-900">
              Edit Expense
            </h3>
            ...
            <div className="flex flex-wrap justify-between gap-2 mt-4">
              ...
              <button
                onClick={handleReset}
                className="flex items-center bg-gray-300 gap-1 px-3 py-1.5 border rounded hover:bg-gray-400"
              >
              ...
              <button
                ...
                className="px-3 py-1.5 text-white bg-indigo-600 border rounded hover:bg-indigo-800"
              >
                Cancel
              </button>
            </div>
